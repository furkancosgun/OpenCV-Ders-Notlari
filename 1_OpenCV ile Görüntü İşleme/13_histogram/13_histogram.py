...
#=HESAPLA([resim],chanel rgb mi yoksa gray scale mi oldugu(grey) ,mask yapılmyor resmin belli kısmını almak
#hsitgroamsize=255 renk skalası 0 da dahil olunca 256,ranges=[0,256]ile arsasında 0 dahil 256 dahil degil)
img_hist = cv2.calcHist([img], channels = [0], mask = None, histSize = [256], ranges = [0,256])
print(img_hist.shape)
plt.figure(), plt.plot(img_hist)


color = ("b", "g", "r")#renkler ayrıştırlmak için tuple içine alınır

plt.figure()
for i, c in enumerate(color):#enumarate color indexini ve degerini alır ve atar
